...
describe('UpdateBuilding Component', () => {
  ...
  it('displays validation errors when submitting an empty form', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText(/Tên toà nhà không được để trống!/i)).toBeInTheDocument();
      expect(screen.getByText(/Mô tả không được để trống!/i)).toBeInTheDocument();
      expect(screen.getByText(/Phường\/Xã không được để trống!/i)).toBeInTheDocument();
    });
  });
  ...
  test('fills input fields with existing building data', async () => {
    const buildingData = {
      ...
      ward: 'Lý Thường Kiệt',
      streetAddress: 'Số 12, Đường Lê Lợi, Phường Lý Thường Kiệt, TP. Quy Nhơn, Bình Định',
      yearBuilt: 2020,
      numberOfFloors: 15,
      area: 500,
      longitude: 108.321,
    ...
    await waitFor(() => {
      expect(screen.getByDisplayValue('Tòa nhà Bình Định Tower')).toBeInTheDocument();
    });
  
    // Kiểm tra các trường khác
    expect(screen.getByDisplayValue('Tòa nhà với tiện nghi hiện đại và an ninh đảm bảo.')).toBeInTheDocument();
  ...
  it('calls the submit handler with correct form data', async () => {
    ...
    fireEvent.change(screen.getByLabelText(/Tên toà nhà/i), { target: { value: 'Toà nhà B' } });
    fireEvent.change(screen.getByLabelText(/Mô tả/i), { target: { value: 'Mô tả toà nhà B' } });

    const submitButton = screen.getByText(/Gửi thông tin/i);
    fireEvent.click(submitButton);

    await waitFor(() => {
      expect(screen.queryByText(/Tên toà nhà không được để trống!/i)).not.toBeInTheDocument();
      expect(screen.queryByText(/Mô tả không được để trống!/i)).not.toBeInTheDocument();
    });
  });
});
